...
Z = zeros(length(Y), max(Y));
for i = 1:length(Y)
	Z(i,Y(i)) = 1;
end

root = split_node(X, Z, Xrange, mean(Y), 1:size(Xrange, 2), 0, depth_limit);
...
if depth == depth_limit || size(Z,1) <= 1 || numel(colidx) == 0
    node.terminal = true;
    node.fidx = [];
    node.fval = [];
    if numel(Z) == 0
...
fprintf('depth %d [%d]: Split on feature %d <= %.2f w/ IG = %.2g (L/R = %d/%d) \n', ...
    depth, length(Z), node.fidx, node.fval, max_ig, numel(leftidx), numel(rightidx));
