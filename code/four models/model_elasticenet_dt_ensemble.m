function [yfit] = model_elasticenet_dt_ensemble(x_train, y_train, x_test)
...
for i = 1:7
    ...
    tree_fit{i} = TreeBagger(35, full(X{i}), residuals{i}, 'method', 'regression', 'Options', tree_stats,'MinLeaf', 2);
    tree_fit{i} = compact(tree_fit{i});
%     svr_fit{i} = train(residuals{i}, X{i}, '-s 13');
