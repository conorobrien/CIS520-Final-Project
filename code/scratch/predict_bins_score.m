function [Y_pred_bins, bins_score, Y_bins] = predict_bins_score(X, Y, X_test, n_bins)
...
%     GBEnsemble = fitensemble(X,Y_bins,'subspace',1000, learner,'type', 'classification');%,'NPredToSample',10);
%     [Y_pred_bins, bins_score] = GBEnsemble.predict(X_test);
%% Decision tree Ensemble
    learner = templateTree();
    GBEnsemble = fitensemble(X,Y_bins,'TotalBoost',20, learner,'type', 'classification');%,'NPredToSample',10);
    [Y_pred_bins, bins_score] = GBEnsemble.predict(X_test);
%% NB
